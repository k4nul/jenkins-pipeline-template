...
checking each preset's output, archive, promotion extraction path, and isolated
single-preset manifest. Each round trip must contain only the requested preset
and its matching selection, preventing a preset-specific execution from
silently falling back to the full matrix. Generated output must stay under
`out/`, which is ignored by Git. Job DSL fixture content is deterministic and
