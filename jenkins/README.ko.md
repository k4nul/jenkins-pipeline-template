...
5. Jenkins 에서 DSL 적용
6. 팀 단위 활성화 전에 `repository-validation` 먼저 실행
...
- 서비스 잡 생성 전에는 `SEED_SKIP_SERVICE_JOBS=true` 가 아닌 한 서비스 파이프라인 검증이 먼저 실행됩니다.
- dry-run 이 아닌 delivery 와 promotion 배포는 Jenkins 승인 프롬프트가 필요하며, 실제 클러스터 배포, Helm 저장소 갱신, bootstrap status 확인은 downstream live rollout 구현이 있을 때만 진행해야 합니다.
...
- `SEED_REPO_URL`
- `SEED_BRANCH_SPEC`
- repository 에 Jenkins SCM credentials 가 필요할 때만 `SEED_SCM_CREDENTIALS_ID`
- `SEED_JOB_ROOT` 같은 선택형 폴더 루트

템플릿은 `main`, `master`, 또는 고정된 protected-branch 정책을 가정하지 않습니다. `SEED_REPO_URL` 은 HTTPS/SSH 저장소 URI 또는 `git@example.invalid:org/repo.git` 같은 Git scp 형식이어야 하며, 로컬 파일 URL 과 상대 경로는 Job DSL 적용 전에 거부됩니다.
